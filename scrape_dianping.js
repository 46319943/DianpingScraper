 ...
 * @param {*} page_index
 */
async function scrapeDianpingID(
  page,
  dianping_id,
  dianping_name,
  time_limit,
  ...
  for (const reviewItem of reviewList) {
    ...
    let resultObject = await reviewItem.evaluate(
      ...
      async (reviewElement) => {
        ...
        // 获取发布时间、评论链接、ID
        resultObject["publish_time"] = reviewElement.querySelector(".time").innerText;
    ...
    resultObject["scrape_time"] = dayjs().format("YYYY-MM-DD HH:mm:ss");

    // 解析时间
    if (resultObject["publish_time"].includes("更新于")) {
      [
        resultObject["first_time"],
        resultObject["publish_time"]
      ] = resultObject["publish_time"].split("更新于");
      resultObject["first_time"] = resultObject["first_time"].trim();
      resultObject["first_timestamp"] = dayjs(resultObject["publish_time"], "YYYY-MM-DD").valueOf();
    } 
    ...
    // 根据时间判断是否需要继续爬取
    if (resultObject["publish_time"] < time_limit) {
      return false;
    }
...
async function insertOrUpdateComment(commentObject) {
  await (
    await mongo.comment
  ).updateOne({ id: commentObject["id"] }, commentObject, { upsert: true });
}
...
async function originImageAdd(){
    // 仅对已有评论的大图的补充
    if (only_picture_supply) {
      let existQueryResult = await (
        await mongo.comment
      ).findOne({ id: resultObject["id"] });
      if (!existQueryResult) {
        continue;
      }
      // 更新大图
